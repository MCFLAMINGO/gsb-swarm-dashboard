...
const NODES: NodeDef[] = [
  ...
  {
    id: "fdot_aadt",
    title: "FDOT Road Traffic",
    org: "Florida DOT",
    icon: Activity,
    color: "orange",
    colorHex: "#f97316",
    vintage: "FDOT Annual Average Daily Traffic 2025 — ArcGIS Layer 7",
    cron: "48h loop · daemon",
    questions: [
      "What is the real vehicle count on roads near this ZIP?",
      "What is the top road in this ZIP by traffic volume?",
      "Is this ZIP viable for a drive-by QSR concept?",
    ],
    signals: ["fdot_max_aadt", "fdot_avg_aadt", "fdot_top_road"],
    demoEndpoint: `${RAILWAY}/api/local-intel/zip-signals/${DEFAULT_ZIP}`,
    demoHeaders: { "x-admin-token": ADMIN_TOKEN },
    status: "live",
  },
  {
    id: "business_signals",
    title: "Business Signal Layer",
    org: "LocalIntel",
    icon: Building2,
    color: "indigo",
    colorHex: "#6366f1",
    vintage: "Derived from businesses + business_tasks — 24h refresh",
    cron: "Daily · daemon",
    questions: [
      "What % of businesses in this ZIP are claimed?",
      "How many have active wallets for payment routing?",
      "What is the food business closure rate?",
      "How many task templates exist per business?",
    ],
    signals: [
      "sig_claimed_rate", "sig_wallet_rate",
      "sig_task_density", "sig_closure_rate_food",
      "biz_density_per_1k",
    ],
    demoEndpoint: `${RAILWAY}/api/local-intel/zip-signals/${DEFAULT_ZIP}`,
    demoHeaders: { "x-admin-token": ADMIN_TOKEN },
  ...
  },
  {
    ...
    color: "yellow",
    colorHex: "#eab308",
    vintage: "Extracts contact emails from business websites statewide",
    cron: "Continuous · daemon",
    questions: [
      "Does this business have a harvestable contact email?",
      "How many businesses have emails in this ZIP?",
    ],
    signals: ["contact_email", "contact_email_source"],
    demoEndpoint: `${RAILWAY}/api/local-intel/zip-signals/${DEFAULT_ZIP}`,
    demoHeaders: { "x-admin-token": ADMIN_TOKEN },
    triggerEndpoint: `${RAILWAY}/api/admin/trigger-website-enricher`,
    triggerLabel: "Trigger Email Harvest",
    status: "live",
  },
  {
    id: "claim_outreach",
    ...
    color: "green",
    colorHex: "#22c55e",
    vintage: "SMS + email outreach to unclaimed businesses — manual trigger only",
    cron: "Manual trigger only",
    questions: [
      "How many unclaimed businesses haven't been contacted in 30 days?",
      "Which businesses have replied CLAIM?",
      "What is the claim conversion rate?",
    ],
    signals: ["sig_claimed_rate", "sig_wallet_rate"],
    demoEndpoint: `${RAILWAY}/api/local-intel/zip-signals/${DEFAULT_ZIP}`,
    demoHeaders: { "x-admin-token": ADMIN_TOKEN },
    triggerEndpoint: `${RAILWAY}/api/admin/trigger-claim-outreach`,
    triggerLabel: "Send Outreach (200 SMS + 200 Email)",
    status: "live",
  },
  {
    id: "mcp_oracle",
